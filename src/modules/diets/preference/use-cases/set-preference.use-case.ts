...
@Injectable()
export class SetDietPreferenceUseCase {
  constructor(
  ...
  async execute(input: SetDietPreferenceInput) {
    ...
    const existingDietPreference = await this.dietPreferenceRepository.findOne(userId)
    ...
    const dietPreference = await this.dietPreferenceRepository.create(userId, input)
